...
function MyFoodOrders() {
    ...
    const deleteBookings = (e) =>{
        ...
        axios.delete(`http://localhost:5000//admin/delete/${orderid}`).then((res) => {
            console.log(res.data)
  ...
  return (
    <div>
        ...
        <Card.Header>{index+1}  <strong>{booking.foodname}</strong> Table</Card.Header>
        <Card.Body>
          <Card.Title>Customer Name : {booking.name}</Card.Title>
